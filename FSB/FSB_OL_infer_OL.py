...
parser.add_argument('--optimizer', choices=['SGD', 'Adam'], help='Choose optimizer', default='Adam')
...
# for mlp
parser.add_argument('--use_mlp', action='store_true', help='if use mlp')
parser.add_argument('--hidden_dim', type=int, default=256, help='hidden dimension for linear classifier')
# AMP
...
_create_info_folder(upper_board_writer, args, files_to_save=["FSB/FSB_OL_infer_OL.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def test(epoch, dataloader, metric):
    ...
    with torch.no_grad():
        for batch_idx, (inputs_ori, inputs_l, _, targets) in enumerate(dataloader):
            inputs_ori, inputs_l, targets = inputs_ori.to(device), inputs_l.to(device), targets.to(device)

            outputs_ori = net(inputs_ori)
            outputs_l = net(inputs_l)

            outputs = torch.cat((outputs_ori, outputs_l), dim=1)
            outputs = net.module.forward_fc(outputs)
...
for idx, subject_out in enumerate(datalist):
   ...
   # model
    net = build_model(args.model, n_classes=n_classes, img_size=args.img_size, n_frames=args.n_frames, pretrained=args.checkpoint, use_mlp=args.use_mlp, n_blocks=2,  hidden_dim=args.hidden_dim, ckpt_in_features=args.ckpt_in_features)
    net = net.to(device)
    if device == 'cuda':
    ...
    testset = MEDataset(test_path_list, test_labels, test_apex_list, n_frames=args.n_frames, transform=transform_test)
...
post_message('#alarm', f'FSB_OL_infer_OL.py / mlp: {args.use_mlp} [{upper_board_writer.log_dir}] done! UF1:{total_uf1}, UAR:{total_uar}, WAR:{total_war}')