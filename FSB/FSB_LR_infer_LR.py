...
_create_info_folder(upper_board_writer, args, files_to_save=["FSB/FSB_LR_infer_LR.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def test(epoch, dataloader, metric):
    ...
    with torch.no_grad():
        for batch_idx, ( _, inputs_l, inputs_r, targets) in enumerate(dataloader):
            inputs_l, inputs_r, targets = inputs_l.to(device), inputs_r.to(device), targets.to(device)

            outputs_ori = net(inputs_l)
