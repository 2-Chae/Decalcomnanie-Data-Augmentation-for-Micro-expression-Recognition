...
_create_info_folder(upper_board_writer, args, files_to_save=["exp3/exp3_OR_infer_OR.py", "dataset.py", "utils/config.py", "models/__init__.py"])
    ...
    for batch_idx, (inputs_ori, _, inputs_r, targets) in enumerate(dataloader):
        ...
        if args.amp:
            with torch.cuda.amp.autocast(): 
                ...
                outputs = torch.cat((outputs_ori, outputs_r), dim=1)
                outputs = net.module.forward_fc(outputs)
...
post_message('#alarm', f'exp3_OR_infer_OR.py / mlp: {args.use_mlp} [{upper_board_writer.log_dir}] done! UF1:{total_uf1}, UAR:{total_uar}, WAR:{total_war}')