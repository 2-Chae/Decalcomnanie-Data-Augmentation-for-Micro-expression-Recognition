...
_create_info_folder(upper_board_writer, args, files_to_save=["FSB/FSB_OLR_infer_OLR.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def test(epoch, dataloader, metric):
    ...
    with torch.no_grad():
        for batch_idx, (inputs_ori, inputs_l, inputs_r, targets) in enumerate(dataloader):
            ...
            outputs = torch.cat((outputs_ori, outputs_l, outputs_r), dim=1)
...
for idx, subject_out in enumerate(datalist):
   ...
   # model
    net = build_model(args.model, n_classes=n_classes, img_size=args.img_size, n_frames=args.n_frames, pretrained=args.checkpoint, use_mlp=args.use_mlp, n_blocks=3,  hidden_dim=args.hidden_dim, ckpt_in_features=args.ckpt_in_features)
    net = net.to(device)
    if device == 'cuda':
