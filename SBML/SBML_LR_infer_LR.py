...
_create_info_folder(upper_board_writer, args, files_to_save=["./exp2/exp2_LR_infer_LR.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def train(dataloader, metric):
    ...
    for batch_idx, (_, inputs_l, inputs_r, targets) in enumerate(dataloader):
        ...
        progress_bar(batch_idx, len(dataloader), 'Loss: %.3f | Acc(l): %.3f%% (%d/%d) | Acc(r): %.3f%% (%d/%d)'
            % (train_loss/(batch_idx+1), 100.*correct_l/total, correct_l, total,100.*correct_r/total, correct_r, total))

    train_acc = 100.*correct_l/total
...
post_message('#alarm', f'exp2_LR_infer_LR.py [{upper_board_writer.log_dir}] done! UF1:{total_uf1}, UAR:{total_uar}, WAR:{total_war}')