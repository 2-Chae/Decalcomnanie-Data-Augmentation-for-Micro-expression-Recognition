...
_create_info_folder(upper_board_writer, args, files_to_save=["./exp2/exp2_OL_infer_OL.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def train(dataloader, metric):
    ...
    for batch_idx, (inputs_ori, inputs_l, _, targets) in enumerate(dataloader):
        ...
        progress_bar(batch_idx, len(dataloader), 'Loss: %.3f | Acc(ori): %.3f%% (%d/%d) | Acc(l): %.3f%% (%d/%d)'
            % (train_loss/(batch_idx+1), 100.*correct_ori/total, correct_ori, total,100.*correct_l/total, correct_l, total))
...
post_message('#alarm', f'exp2_OL_infer_OL.py [{upper_board_writer.log_dir}] done! UF1:{total_uf1}, UAR:{total_uar}, WAR:{total_war}')