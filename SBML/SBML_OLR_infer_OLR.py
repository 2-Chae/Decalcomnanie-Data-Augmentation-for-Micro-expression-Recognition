...
_create_info_folder(upper_board_writer, args, files_to_save=["./exp2/exp2_OLR_infer_OLR.py", "dataset.py", "utils/config.py", "models/__init__.py"])
...
def test(epoch, dataloader, metric):
    ...
    with torch.no_grad():
        for batch_idx, (inputs_ori, inputs_l, inputs_r, targets) in enumerate(dataloader):
            inputs_ori, inputs_l, inputs_r, targets = inputs_ori.to(device), inputs_l.to(device), inputs_r.to(device), targets.to(device)

            outputs_ori = net(inputs_ori)
            outputs_l = net(inputs_l)
            outputs_r = net(inputs_r)

            outputs = outputs_ori + outputs_l + outputs_r 
    ...
    for batch_idx, (inputs_ori, inputs_l, inputs_r, targets) in enumerate(dataloader):
        ...
        if args.amp:
            with torch.cuda.amp.autocast(): 
                ...
                outputs_l = net(inputs_l)
                loss_l = criterion(outputs_l, targets)

                outputs_r = net(inputs_r)
                loss_r = criterion(outputs_r, targets)

                loss = 0.4 * loss_ori + 0.4 * loss_l + 0.2 * loss_r
        else:
            outputs_ori = net(inputs_ori)
            loss_ori = criterion(outputs_ori, targets)
        ...
        _, predicted = outputs_l.max(1)
        correct_l += predicted.eq(targets).sum().item()
        ...
        progress_bar(batch_idx, len(dataloader), 'Loss: %.3f | Acc(ori): %.3f%% (%d/%d) | Acc(l): %.3f%% (%d/%d) | Acc(r): %.3f%% (%d/%d)'
            % (train_loss/(batch_idx+1), 100.*correct_ori/total, correct_ori, total,100.*correct_l/total, correct_l, total, 100.*correct_r/total, correct_r, total))
...
post_message('#alarm', f'exp2_OLR_infer_OLR.py [{upper_board_writer.log_dir}] done! UF1:{total_uf1}, UAR:{total_uar}, WAR:{total_war}')